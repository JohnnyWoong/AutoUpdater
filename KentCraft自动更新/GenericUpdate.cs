...
namespace winform自动更新
{
    public class GenericUpdate : Form
    {
        ...
        private void PromptUpdateRootNotification()
        {
            ...
            //如果有多个启动器,则打开最新的启动器
            var exe = Jnw.Common.FileHelper.GetFileNames(_winformPath, "KentCraft智能启动器 v*.exe", false);
            if (exe.Length > 1)
            {
                string newExe = "";
                ...
                {
                    if (newExe == "" ||
                        Convert.ToInt32(temp.Substring((_winformPath + "KentCraft智能启动器 v").Length, 6)) >
                        Convert.ToInt32(newExe.Substring((_winformPath + "KentCraft智能启动器 v").Length, 6)))
                        newExe = temp;
                }
