    ...
    public partial class CheckUpdate2 : GenericUpdate
    {
        ...
        protected void CheckUpdate_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string newest = "";
                //获取所有更新器
                var newExe = FileHelper.GetFileNames(WinformPath, "KentCraft智能启动器 v*.exe", false);
                //获取最新更新器
                foreach (var temp in newExe)
                {
                    if (newest == "" || Convert.ToInt32(temp.Substring((WinformPath + "KentCraft智能启动器 v").Length, 6)) > Convert.ToInt32(newest.Substring((WinformPath + "KentCraft智能启动器 v").Length, 6)))
                    {
                        newest = temp;
                    }
                }
                ...
                while (newExe.Length > 1 && Application.ExecutablePath.Contains(FileHelper.GetFileName(newest)))
                {
                    ...
                    var exe = FileHelper.GetFileNames(WinformPath, "KentCraft智能启动器 v*.exe", false).Where(i =>
                    Convert.ToInt32(i.Substring((WinformPath + "KentCraft智能启动器 v").Length, 6)) <=
                    Convert.ToInt32(Application.ExecutablePath.Substring((WinformPath + "KentCraft智能启动器 v").Length, 6)));
                    foreach (var temp in exe)
                    {
                        if (!Application.ExecutablePath.Contains(FileHelper.GetFileName(temp)))
                            FileHelper.DeleteFile(temp);
                    }
                    newExe = FileHelper.GetFileNames(WinformPath, "KentCraft智能启动器 v*.exe", false);
                }
                //删除冗余文件
