    ...
    public class AutoUpdate
    {
        ...
        public bool UpdateFile(BackgroundWorker bgw, ProgressBar pb)
        {
            ...
            try
            {
                ...
                foreach (string ss in fl)
                {
                    ...
                    else
                    {
                        if (!string.IsNullOrWhiteSpace(ss))
                        {
                            ...
                            {
                                //更新启动器--调试时需注释
                                if (operateFile.IndexOf("new") == 0 && Convert.ToInt32(ss.Substring(FileHelper.GetContent("KCLconfig.ini")[4].Length, 6)) > Convert.ToInt32(Application.ExecutablePath.Substring(Application.ExecutablePath.IndexOf(FileHelper.GetContent("KCLconfig.ini")[4]) + FileHelper.GetContent("KCLconfig.ini")[4].Length, 6)))
                                {
                                    wc = new WebClient();
                                    ...
                                    if (localPath.Contains(FileHelper.GetContent("KCLconfig.ini")[4]))
                                    {
                                        var tempFile = FileHelper.GetFileNames(_winformPath + "Temp\\", FileHelper.GetContent("KCLconfig.ini")[4] + "*.exe", false);
                ...
                pb.Invoke(pbNow, 0);
                pb.Invoke(pbMax, 4);
                FileHelper.CopyFolder(_winformPath + "Temp\\", _winformPath, FileHelper.GetContent("KCLconfig.ini")[4]);
                pb.Invoke(pbNow, 1);
                //删除目录失败不影响整体更新
                try
                {
            ...
            catch (Exception e)
            {
                ...
                if (tempFile.Length == 1)
                {
                    ...
                    //打开更新的更新器
                    Process.Start(FileHelper.GetFileNames(_winformPath, FileHelper.GetContent("KCLconfig.ini")[4] + "*.exe", false).First());
                    Application.Exit();
                }
                else
                {
